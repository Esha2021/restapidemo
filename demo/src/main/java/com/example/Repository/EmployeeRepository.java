...
public interface EmployeeRepository extends JpaRepository<EmployeeDetail,Long> {
    ...
    @Modifying
    @Transactional
    @Query("Update  EmployeeDetail set employeeName=:employeeName where employeeId=:employeeId")
    Integer updateEmployeeName(@Param("employeeId") Long employeeId, @Param("employeeName") String employeeName);
}